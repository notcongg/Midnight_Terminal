...
def _extract_quoted_value(text: str, start: int) -> tuple[str, int]:
    """
    ...
    raise ValueError("env: unterminated single quote in .midconf")
...
def _store_assignment(
    ...
    if value.startswith("'"):
        ...
        value, _ = _extract_quoted_value(assignment, eq_idx + 1)
...
def _parse_alias_statement(
    ...
    """
    Parse an alias statement: alias name=command
    Returns (name, command) or None if not a valid alias statement.
    """
...
def _load_midconf(
    ...
    # Reloads replace the full config state: aliases are re-read
    # from .midconf, so stale aliases from a previous load are dropped.
    context.aliases = {}
    ...
    while index < len(lines):
        ...
        stripped = _strip_line_comment(stripped).strip()

        if not stripped:
            index += 1
            continue

        # Handle set statement (possibly multiline quoted value)
        if stripped.startswith("set "):
            # Check if value is a single-quoted multiline value
            eq_pos = stripped.find("=")
            if eq_pos > 0:
                after_eq = stripped[eq_pos + 1:].strip()
                if after_eq.startswith("'") and after_eq.count("'") < 2:
                    # Multiline quoted value - collect lines until closing quote
                    collected = line
                    next_idx = index + 1
                    while next_idx < len(lines) and "'" not in collected.split("=", 1)[1].strip()[1:]:
                        collected += "\n" + lines[next_idx]
                        next_idx += 1

                    _store_assignment(collected, context)
        ...
        if stripped.startswith("alias "):
            parsed = _parse_alias_statement(stripped)
            if parsed:
                context.aliases[parsed[0]] = parsed[1]

            index += 1
            continue

        # Handle command (non-comment, non-set, non-alias)
        _execute_config_command(stripped, context)
